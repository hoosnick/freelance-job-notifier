...
async def create_offer(request: Request):
    ...
    new_offer = Offer(
        project=project, offer=result.get("response"), offer_by=parsed_user
    )
...
async def parse_user(telegram_id: int):
    return await PremiumUser.objects().get(PremiumUser.tg_id == telegram_id)
