...
@router.message(
    CommandStart(
        deep_link=True, magic=F.args.regexp(re.compile(r"INVITE_([A-Z0-9]{6})"))
    )
)
async def new_premium_user(m: Message, command: CommandObject):
    ...
    pro_user = await PremiumUser.objects().get(PremiumUser.invite_link == invite_link)
...
async def generate_promo(m: Message, config: Settings):
    ...
    return m.answer(
        f"<code>https://t.me/{config.bot_username}?start={invite_code}</code>"
    )
